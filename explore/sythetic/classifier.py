...
Simple  power-law fit  f(x) = a·x^b + c
          R² ≥ 0.85  &  |b−1| ≤ 0.05  → Linear
          R² ≥ 0.85  &  |b−1| ≥ 0.5   → Saturation (b<1) / Acceleration (b>1)
...
DEFAULTS: dict[str, Any] = {
    ...
    "dcor_strong": 0.7,
    "dcor_intermediate": 0.5,
    "pearson_threshold": 0.7,
    "r2_power_threshold": 0.85,
    "linear_b_tolerance": 0.05,
    "curvature_b_threshold": 0.5,
    "branch_n_bins": 10,
    "branch_frac_threshold": 0.25,
...
def classify_relationship(
    ...
    *,
    ...
    params : dict, optional
        Override any key in DEFAULTS. Use ``gate1_metric="dcor"``
        to switch from MIC to distance correlation.
    """
    ...
    score = gate1_score if gate1_score is not None else mic
    if score is None:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            score = compute_gate1_score(x, y, metric)
