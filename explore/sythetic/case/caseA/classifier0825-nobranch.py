...
Decision tree
─────────────
Gate 1  score ≥ strong   → strong global        (score = MIC or dCor)
        score ≥ intermediate → intermediate global
        score < intermediate → no global → "No Global Relationship"
...
DEFAULTS: dict[str, Any] = {
    "gate1_metric": "dcor",   #dcor  mic
    "mic_strong": 0.8,
    "mic_intermediate": 0.4,
    "dcor_strong": 0.8,
    "dcor_intermediate": 0.4,
    "pearson_threshold": 0.7,
    "r2_power_threshold": 0.5,

    "r2_autocorr_shortcut": True,
    "shortcut_autocorr_max": 0.7,

    "linear_b_tolerance": 0.3,
    "curvature_b_threshold": 0.3,
    ...
    "sizer_flat_ref": "max",
    "tp_support_min": 2,
    "force_complex": False,    #直接全部走complex
}
...
def classify_relationship(
    ...
    *,
    ...
    ----------
    mic : float, optional
        Pre-computed MIC value (backward-compatible shortcut).
    gate1_score : float, optional
        Pre-computed gate-1 score. Takes precedence over *mic*.
    params : dict, optional
        Override any key in DEFAULTS. Both MIC and dCor are always
        computed and returned. ``gate1_metric`` (``"mic"`` or ``"dcor"``)
        only chooses which score opens the next gate.
    """
    ...
    if path == "Simple":
        ...
        if label is not None:
            ...
            diagnostics["path_trace"] = (
                f"{metric.upper()}:{gate1} -> Pearson:Simple -> Power:{label}"
            )
            return _result(
                label, "Yes", "Simple", "N/A", None, score, metric,
                **diagnostics,
            )

    diagnostics["complex_path_used"] = True
    complex_trace = f"{metric.upper()}:{gate1} -> Pearson:{path}"
    if path == "Simple":
        complex_trace += (
    ...
    if is_branch:
        ...
        return _result(
            "Branch", "Yes", path, "Yes", None, score, metric,
            **diagnostics,
        )
    ...
    if tp >= 3:
        ...
        return _result(
            "Oscillation", "Yes", path, "No", tp, score, metric,
            **diagnostics,
        )
    ...
    if is_transition:
        ...
        return _result(
            "Transition", "Yes", path, "No", 0, score, metric,
            **diagnostics,
        )
    ...
    return _result(
        "Uncertain", "Yes", path, "No", 0, score, metric,
        **diagnostics,
    )
...
def classify_batch(
    ...
    Optional: ``'name'`` (label), ``'gate1_score'`` or ``'mic'``
    (pre-computed gate-1 value).
    """
    rows = []
    for case in cases:
