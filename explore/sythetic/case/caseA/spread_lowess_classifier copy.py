...
CLASSIFIER_VERSION = "eq15-detrended-spread-lowess-v7-independent-axes"
...
# Independent classification axes: strength, direction, and structure.
...
PEAK_DROP_THRESHOLD = 0.10
TROUGH_DROP_THRESHOLD = 0.15
TROUGH_RISE_THRESHOLD = 0.10
NET_EDGE_BINS = 2
...
def classify_spread(
    ...
    *,
    ...
    Strength, direction, and structure are classified independently.  The
    legacy ``spread_label`` is retained as a compact plotting label, while the
    independent fields preserve combinations such as
    ``Variable | Decreasing | Interior peak``.
    """
    ...
    clean_peak = bool(
        0 < peak_index < len(smooth) - 1
        and peak_rise >= peak_rise_threshold
        and peak_drop >= peak_drop_threshold
    ...
    clean_trough = bool(
        0 < trough_index < len(smooth) - 1
        and trough_drop >= trough_drop_threshold
        and trough_rise >= trough_rise_threshold
    ...
    # Axis 1: strength.  Net change prevents a smooth but persistent trend from
    # being called Stable merely because its relative amplitude is modest.
    stable = bool(
    ...
    # Axis 2: global direction. TP and reversal are deliberately excluded;
    # they describe internal structure, not the net direction of the profile.
    if (
        net_change >= direction_net_change_threshold
        and pearson >= pearson_threshold
        and spearman >= pearson_threshold
    ):
        direction, direction_confidence = "Increasing", "Strong"
    elif (
        net_change <= -direction_net_change_threshold
        and pearson <= -pearson_threshold
        and spearman <= -pearson_threshold
    ):
        direction, direction_confidence = "Decreasing", "Strong"
    elif (
        net_change >= direction_net_change_threshold
        and pearson > 0
        and spearman > 0
        and max(pearson, spearman) >= pearson_threshold
    ):
        direction, direction_confidence = "Increasing", "Candidate"
    elif (
        net_change <= -direction_net_change_threshold
        and pearson < 0
        and spearman < 0
        and min(pearson, spearman) <= -pearson_threshold
    ):
        direction, direction_confidence = "Decreasing", "Candidate"
    else:
        direction, direction_confidence = "No dominant direction", "Not applicable"

    # Axis 3: internal structure.  A peak/trough may coexist with a strong
    # global direction.  Raw TP count remains diagnostic only; cumulative
    # side coherence keeps small LOWESS wiggles from creating a structure.
    if stable:
        structure = "Simple"
    elif clean_peak and clean_trough:
        if peak_prominence >= structure_dominance_ratio * trough_prominence:
            structure = "Interior peak"
        elif trough_prominence >= structure_dominance_ratio * peak_prominence:
    ...
    structure_group = "Simple" if structure == "Simple" else "Complex"

    # Compact legacy label used only where one colour/label is required.
    if stable:
        label = "Stable"
    elif structure != "Simple":
        label = structure
    elif direction == "No dominant direction":
        label = "Simple"
    elif direction_confidence == "Candidate":
        label = f"Candidate {direction}"
    else:
        label = direction
    display_direction = (
        f"Candidate {direction}"
        if direction_confidence == "Candidate" else direction
    )
    display_label = f"{strength} | {display_direction} | {structure}"
