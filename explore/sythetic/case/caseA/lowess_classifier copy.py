...
分类结果包含：响应强度 (R²)、一级形状 (R_rev + Pearson)、
...
# ---- 单调性判定 ----
RREV_THRESH = 0.05           # 反向变化占总变化的最大允许比例
PEARSON_MONO_THRESH = 0.50   # R_rev 小时，|Pearson| 低于此值列为 Candidate
...
def classify_panel(x, y, xs, ys,
    ...
    rrev_thresh : 反向变化比例阈值；达到阈值则为 Non-monotonic
    pearson_thresh : R_rev 小时确认 Monotonic 的 |Pearson| 阈值
    ...
    # ---- Step 7: TP 仅作为 Non-monotonic 的子形状诊断 ----
    n_tp = _count_turning_points(d, deadzone)
    out["ls_n_tp"] = n_tp

    # ==== 分支 A: R_rev 小，总体方向一致 ====
    if out["ls_R_rev"] < rrev_thresh:
        ...
        prefix = "monotonic" if abs(pr) >= pearson_thresh else "candidate_monotonic"
